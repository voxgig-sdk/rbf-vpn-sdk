...
const TEST_SEED = { "entity": { "subscription": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+RbfVpnSDK\b/.test(code) ||
        /\bRbfVpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+RbfVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/RbfVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
