...
class RbfVpnTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RbfVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RbfVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RbfVpnTestRunner.getenv("RBF_VPN_TEST_LIVE")
        override = RbfVpnTestRunner.getenv("RBF_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RbfVpnTestRunner.getenv(key)
        ...
        explain = RbfVpnTestRunner.getenv("RBF_VPN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["RBF_VPN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RbfVpnTestRunner._test_control is not None:
            return RbfVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RbfVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RbfVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RbfVpnTestRunner.load_test_control()
...
def load_env_local():
    RbfVpnTestRunner.load_env_local()


def env_override(m):
    return RbfVpnTestRunner.env_override(m)


def entity_data(v):
    return RbfVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RbfVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RbfVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RbfVpnTestRunner.load_test_control()


def live_client_options():
    return RbfVpnTestRunner.live_client_options()


def live_delay_ms():
    return RbfVpnTestRunner.live_delay_ms()
