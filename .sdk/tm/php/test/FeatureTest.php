...
class FtCtrl extends RbfVpnControl
{
...
class FtClient
{
    ...
    private RbfVpnUtility $utility;

    public function __construct(RbfVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): RbfVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RbfVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RbfVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RbfVpnConfig::make_feature($name);
    ...
    private static function buildUrl(RbfVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RbfVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RbfVpnResponse($fetched);
        }
    ...
    private function fail(RbfVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RbfVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
