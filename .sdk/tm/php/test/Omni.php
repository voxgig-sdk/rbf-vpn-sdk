...
//    only - a typed RbfVpnContext there would make every ctx
...
final class RbfVpnOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RbfVpnContext($clean, null);
    ...
    public static function err_from_map($m): ?RbfVpnError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RbfVpnError(is_string($code) ? $code : '', $msg);
    }
