...
// RbfVpn SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class RbfVpnMakeError
{
    public static function call(?RbfVpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RbfVpnContext([], null);
        }
        $op = $ctx->op ?? new RbfVpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RbfVpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RbfVpnError) ? $err->msg : (string)$err;
        $msg = "RbfVpnSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new RbfVpnError('', $msg, $ctx);
