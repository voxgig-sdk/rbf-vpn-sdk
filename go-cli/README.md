...
boru-driven command-line client **and** interactive REPL for the RbfVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rbf-vpn-cli)
make build
...
./rbf-vpn-cli load 1 subscription            # {id:1} shorthand
./rbf-vpn-cli load '{id:1}' subscription       # explicit match map

# 5. Override the API base URL for a single call
RBF_VPN_BASE=https://api.example.com ./rbf-vpn-cli load 1 subscription

# 6. No arguments -> interactive REPL
./rbf-vpn-cli
rbf-vpn> load 1 subscription
rbf-vpn> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rbf-vpn-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/rbf-vpn-cli load 1 subscription
   ```
...
```sh
./rbf-vpn-cli load 1 subscription          # scalar shorthand for {id:1}
./rbf-vpn-cli load '{id:1}' subscription     # explicit match map
```
...
./rbf-vpn-cli load 1 subscription
```
...
Run with no arguments to open a REPL (prompt `rbf-vpn>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `subscription`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/rbf-vpn-cli`. |
...
(`./rbf-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the
