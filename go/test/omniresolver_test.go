...
import (
	...
	omni "github.com/voxgig-sdk/rbf-vpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RbfVpnError); is {
				out := map[string]any{
					"name":    "RbfVpnError",
...
func omniCtx(arg any, client *sdk.RbfVpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RbfVpnSDK); is {
			client = live
