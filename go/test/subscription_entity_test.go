...
func TestSubscriptionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RBF_VPN_TEST_SUBSCRIPTION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func subscriptionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RBF_VPN_TEST_SUBSCRIPTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RBF_VPN_TEST_SUBSCRIPTION_ENTID"])
	...
	if env["RBF_VPN_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewRbfVpnSDK(core.ToMapAny(mergedOpts))
	}
