# RbfVpn SDK Subscription entity
...
class SubscriptionEntity
  ...
  def data_set(args)
    if args
      @_data = RbfVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RbfVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Subscription, Hash] the loaded Subscription; raises RbfVpnError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RbfVpnHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
