...
module RbfVpnUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RbfVpnContext.new({}, nil)
    end
    op = ctx.op || RbfVpnOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RbfVpnResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RbfVpnError) ? err.msg : err.to_s
    ...
    sdk_err = RbfVpnError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RbfVpnError)
