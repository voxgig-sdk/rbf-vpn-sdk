...
class SubscriptionEntity
{
    ...
    {
        if ($args) {
            $this->_data = RbfVpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = RbfVpnHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws RbfVpnError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RbfVpnHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
