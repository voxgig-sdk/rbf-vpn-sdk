...
type Args struct {
	Entity string         `json:"entity" jsonschema:"subscription"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RbfVpnSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "rbf-vpn_list",
		Description: "List records from RbfVpn. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "rbf-vpn_load",
		Description: "Load a single record from RbfVpn. " +
...
func runOp(client *sdk.RbfVpnSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RbfVpnSDK, name string) (sdk.RbfVpnEntity, error) {
