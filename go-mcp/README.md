...
[MCP](https://modelcontextprotocol.io) server exposing the RbfVpn SDK as
two agent tools — `rbf-vpn_list` and `rbf-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rbf-vpn-mcp)
make build

# 2. Provide credentials via the environment
export RBF_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user rbf-vpn \
  -- /absolute/path/to/rbf-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./rbf-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// rbf-vpn_list: first page of records
{ "entity": "subscription" }
{ "entity": "subscription", "query": { } }

// rbf-vpn_load: one record by id
{ "entity": "subscription", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rbf-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user rbf-vpn \
     -- "$PWD"/dist/*/rbf-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `rbf-vpn_list` and `rbf-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list subscription using rbf-vpn"*
   and it calls `rbf-vpn_list` with `{"entity":"subscription"}`.
...
```sh
export RBF_VPN_APIKEY=sk_live_xxx            # API key
export RBF_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `rbf-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `rbf-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RBF_VPN_APIKEY` | API key sent with every request. |
| `RBF_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./rbf-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"rbf-vpn_load","arguments":{"entity":"subscription","query":{"id":1}}}}'
```
